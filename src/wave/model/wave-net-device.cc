...
WaveNetDevice::SendX (Ptr<Packet> packet, const Address & dest, uint32_t protocol, const TxInfo & txInfo)
{
  ...
  if (!IsAvailableChannel (txInfo.channelNumber))
    {
      std::cout << "not Available Channel" << std::endl;
      return false;
    }
  if (!m_channelScheduler->IsChannelAccessAssigned (txInfo.channelNumber))
    {
      std::cout << "there is no channel access assigned for channel" << std::endl;
  ...
  if ((txInfo.channelNumber == CCH) && (protocol == IPv4_PROT_NUMBER || protocol == IPv6_PROT_NUMBER))
    {
      std::cout << "IP-based packets shall not be transmitted on the CCH" << std::endl;
      NS_LOG_DEBUG ("IP-based packets shall not be transmitted on the CCH");
      return false;
    }
  if ((txInfo.priority > 7) || txInfo.txPowerLevel > 8)
    {
      std::cout <<"invalid transmit parameters."<< std::endl;
  ...
  if ((txInfo.dataRate == WifiMode ()) || (txInfo.txPowerLevel == 8))
    {
      std::cout <<"High layer does not want to control tx parameters."<< std::endl;
      // let MAC layer itself determine tx parameters.
      NS_LOG_DEBUG ("High layer does not want to control tx parameters.");
    }
  else
      ...
      for (PhyEntitiesI i = m_phyEntities.begin (); i != m_phyEntities.end (); ++i)
        {
          if ( !((*i)->IsModeSupported (txInfo.dataRate)))
            {
              std::cout << "if current PHY devices do not support data rate of the  tx profile" << std::endl;
              return false;
            }
        }
...
uint32_t 
WaveNetDevice::GetLocalDensity ()
{
  return m_localDensity;
}

void
WaveNetDevice::SetLocalDensity (uint32_t localDensity)
{
  m_localDensity = localDensity;
}

void
WaveNetDevice::CalculateLocalDensity (uint32_t var)
{
  m_localDensity = var;
}

uint8_t
WaveNetDevice::CalculateTxPower ()
{
  return m_localDensity%7;
}

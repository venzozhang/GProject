...
void
LteHelper::AddD2DLink(NetDeviceContainer ueDevices, NetDeviceContainer enbDevices)
{
  // ziji dianjia 
  Ptr<LteEnbNetDevice> enbDev = DynamicCast<LteEnbNetDevice>(enbDevices.Get(0));
  Ptr<LteUeNetDevice> ueDev1 = DynamicCast<LteUeNetDevice>(ueDevices.Get(0));
  Ptr<LteUeNetDevice> ueDev2 = DynamicCast<LteUeNetDevice>(ueDevices.Get(1)); 
  //std::cout << ueDev1.GetTypeId() << std::endl;

  Ptr<LteUePhy> phy1 = ueDev1 -> GetPhy();
  Ptr<LteUePhy> phy2 = ueDev2 -> GetPhy();

  uint16_t ulEarfcn = enbDev->GetUlEarfcn();
  uint8_t  ulBandwidth = enbDev->GetUlBandwidth();

  Ptr<LteEnbPhy> phyEnb = enbDev -> GetPhy();
  double   noiseFigure = phyEnb-> GetNoiseFigure();


  Ptr<SpectrumValue> noisePsd = LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (ulEarfcn, ulBandwidth, noiseFigure);
  
  phy1->GetUlSpectrumPhy()->SetNoisePowerSpectralDensity (noisePsd);
  phy2->GetUlSpectrumPhy()->SetNoisePowerSpectralDensity (noisePsd);

  m_uplinkChannel->AddRx (phy1->GetUlSpectrumPhy());
  m_uplinkChannel->AddRx (phy2->GetUlSpectrumPhy());
}

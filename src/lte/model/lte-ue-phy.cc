...
//********************************************************************************************************************************************
  m_d2dsendlinkSpectrumPhy = CreateObject<LteSpectrumPhy> ();
  m_d2drecvlinkSpectrumPhy = CreateObject<LteSpectrumPhy> ();
  m_useD2DMode = false;
  //********************************************************************************************************************************************
...
LteUePhy::DoSendMacPdu (Ptr<Packet> p)
{
  ...
  p->PeekHeader (rrcUlDcchMessage);

  // NS_LOG_UNCOND("LteUePhy::DoSendMacPdu:  message type: "  << rrcUlDcchMessage.GetMessageType() );
  ...
  if(tag.GetD2DFlag())
  {
    NS_LOG_UNCOND("LteUePhy::DoSendMacPdu() has received the D2D packet from Mac ");
    SetD2DMacPdu (p);
  }
  else
  {
...
{
    // NS_LOG_UNCOND("LteUePhy::PhyPduReceived" <<m_test);
...
LteUePhy::SetSubChannelsForTransmission (std::vector <int> mask)
{
  ...
  //********************************************************************************************************************************************
  m_d2dsendlinkSpectrumPhy -> SetTxPowerSpectralDensity (txPsd);
  //********************************************************************************************************************************************
  ...
  if (m_ulConfigured)
    {
      ...
      if (pb)
        {
          // std::cout << " UE UL has data to transport, here transport data and control" << std:: endl;
          if (m_enableUplinkPowerControl)
            {
        ...
        {
          // std::cout << " UE UL only has control to transport" << std:: endl;
    ...
    //********************************************************************************************************************************************
      if(m_useD2DMode)
       { 
            Ptr<PacketBurst> d2dPb = GetD2DPacketBurst ();
            if(d2dPb)
            {
               m_d2dsendlinkSpectrumPhy -> StartTxDataFrame (d2dPb, ctrlMsg, UL_DATA_DURATION);
               NS_LOG_UNCOND("LteUePhy::SubframeIndication() " << "send d2d packet" );
            }
       }
...
{
  NS_LOG_FUNCTION (this);
  //NS_LOG_UNCOND("LteUePhy::DoReset ()  " << Simulator::Now());
  m_rnti = 0;
  ...
  for (int i = 0; i < m_macChTtiDelay; i++)
    {
      ...
      m_d2dPacketBurstQueue.push_back (d2dPb);
      /******************************************************************************************************/
    }
  std::vector <int> ulRb;
...
Ptr<LteSpectrumPhy> 
LteUePhy::GetD2DSendSpectrumPhy()
{
  return m_d2dsendlinkSpectrumPhy;
}

Ptr<LteSpectrumPhy> 
LteUePhy::GetD2DRecvSpectrumPhy()
{
  return m_d2drecvlinkSpectrumPhy;
}

void
LteUePhy::DoUseD2DMode(bool flag, uint64_t targetImsi,  uint16_t targetRnti)
{
...
if(flag) 
{
    ...
    for (NodeList::Iterator i = NodeList::Begin (); (i != listEnd) && (!found); ++i)
      ...
      if( found)
      {
          Ptr<LteSpectrumPhy> dstD2DSendSpectrum = ueDev->GetPhy() -> GetD2DSendSpectrumPhy();
          Ptr<LteSpectrumPhy> dstD2DRecvSpectrum = ueDev->GetPhy() -> GetD2DRecvSpectrumPhy();

          ObjectFactory channelFactory;
          channelFactory.SetTypeId (MultiModelSpectrumChannel::GetTypeId ());

          m_d2dChannel1 = channelFactory.Create<SpectrumChannel> ();
          m_d2dChannel2 = channelFactory.Create<SpectrumChannel> ();

          Ptr<SpectrumValue> noisePsd = LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (5, 5, 5);
  
          dstD2DRecvSpectrum -> SetNoisePowerSpectralDensity (noisePsd);
          m_d2drecvlinkSpectrumPhy -> SetNoisePowerSpectralDensity (noisePsd);

          m_d2dsendlinkSpectrumPhy->SetChannel( m_d2dChannel1 );
          m_d2dChannel1 -> AddRx( dstD2DRecvSpectrum );

          dstD2DSendSpectrum->SetChannel( m_d2dChannel2 );
          m_d2dChannel2 -> AddRx( m_d2drecvlinkSpectrumPhy );

          //phy->SetDevice (dev);
          dstD2DSendSpectrum->SetDevice (ueDev);
          dstD2DRecvSpectrum->SetDevice (ueDev);

          m_d2dsendlinkSpectrumPhy->SetDevice (GetDevice());
          m_d2drecvlinkSpectrumPhy->SetDevice (GetDevice());


          m_d2drecvlinkSpectrumPhy->SetLtePhyRxDataEndOkCallback (MakeCallback (&LteUePhy::PhyPduReceived, this));
          m_d2drecvlinkSpectrumPhy->SetLtePhyRxCtrlEndOkCallback (MakeCallback (&LteUePhy::ReceiveLteControlMessageList, this));

          dstD2DRecvSpectrum->SetLtePhyRxDataEndOkCallback (MakeCallback (&LteUePhy::PhyPduReceived, ueDev->GetPhy()));
          dstD2DRecvSpectrum->SetLtePhyRxCtrlEndOkCallback (MakeCallback (&LteUePhy::ReceiveLteControlMessageList, ueDev->GetPhy()));


           m_d2drecvlinkSpectrumPhy -> AddExpectedTb(m_rnti, true, 2, 2, std::vector<int> (), 0, 2, 2, true);
           dstD2DRecvSpectrum -> AddExpectedTb(targetRnti, true, 2, 2, std::vector<int> (), 0, 2, 2, true);

           m_d2dsendlinkSpectrumPhy->UseD2DMode(flag);
           m_d2drecvlinkSpectrumPhy->UseD2DMode(flag);
           dstD2DRecvSpectrum->UseD2DMode(flag);
           dstD2DSendSpectrum->UseD2DMode(flag);

          //NS_LOG_UNCOND("LteUePhy::DoUseD2DMode () : setup d2d channel");

      }
      else {
        NS_FATAL_ERROR("LteUePhy::DoUseD2DMode()  ----->>>> the target imsi didn't exist ");
      }
...
/*****************************************************************************************************/
void
LteUePhy::SetD2DMacPdu (Ptr<Packet> p)
{
  m_d2dPacketBurstQueue.at (m_d2dPacketBurstQueue.size () - 1)->AddPacket (p);
}

Ptr<PacketBurst>
LteUePhy::GetD2DPacketBurst (void)
{
  if (m_d2dPacketBurstQueue.at (0)->GetSize () > 0)
    {
      Ptr<PacketBurst> ret = m_d2dPacketBurstQueue.at (0)->Copy ();
      m_d2dPacketBurstQueue.erase (m_d2dPacketBurstQueue.begin ());
      m_d2dPacketBurstQueue.push_back (CreateObject <PacketBurst> ());
      return (ret);
    }
  else
    {
...
//  code writen by Zhao Jianwu below is not consider macChTtiDelay, I has used the code version above.

// void
// LtePhy::SetD2DMacPdu(Ptr<Packet> p)
// {
//   if(m_d2dPacketBurstQueue.size() == 0)
//   {
//     Ptr<PacketBurst> pb = CreateObject<PacketBurst>();
//     m_d2dPacketBurstQueue.push_back(pb);
//   }
//   m_d2dPacketBurstQueue.at (0)->AddPacket (p);

// }

// Ptr<PacketBurst>
// LtePhy::GetD2DPacketBurst(void)
// {
//   if(m_d2dPacketBurstQueue.size() == 0)
//   {
//     return (0);

...
//********************************************************************************************************************************************
void
UeManager::RecvD2DConnectionRequest(LteRrcSap::D2DConnectionRequest msg)
{
  // NS_ASSERT( m_state == CONNECTION_SETUP );

  LteRrcSap::D2DConnectionSetup setupMsg;

  // std::map<uint16_t, Ptr<UeManager> >::const_iterator it =m_rrc -> m_ueMap.find (m_rnti);

  std::map<uint16_t, Ptr<UeManager> >::const_iterator it ;

  for( it = m_rrc -> m_ueMap.begin(); it != m_rrc -> m_ueMap.end(); ++it)
  {
    if( it->second -> m_imsi == msg.dstUeIdentity)
    {
      NS_LOG_UNCOND ("UeManager::RecvD2DConnectionRequest ----  target rnti:" << it->second->m_rnti);
      setupMsg.rnti = it->second->m_rnti;
      break;
    }
  }

    if( it == m_rrc -> m_ueMap.end() )
    {
      NS_LOG_UNCOND ("eManager::RecvD2DConnectionRequest   target ue not exist" );
      return;
    }


  m_rrc->m_rrcSapUser->SendD2DConnectionSetup (m_rnti, setupMsg);
  m_rrc->m_rrcSapUser->SendD2DConnectionSetup (setupMsg.rnti, setupMsg);
}
...
{
  NS_LOG_FUNCTION (this << rnti);
  NS_LOG_UNCOND("LteEnbRrc::DoRecvRrcConnectionRequest ");
  GetUeManager (rnti)->RecvRrcConnectionRequest (msg);
}

//********************************************************************************************************************************************
void
LteEnbRrc::DoRecvD2DConnectionRequest (uint16_t rnti, LteRrcSap::D2DConnectionRequest msg)
{
  NS_LOG_FUNCTION (this << rnti);
  GetUeManager (rnti)->RecvD2DConnectionRequest (msg);
  //std::cout<< "received D2DConnectionRequest:  target : " <<msg.dstUeIdentity<< std::endl;
}
//********************************************************************************************************************************************

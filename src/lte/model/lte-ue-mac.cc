...
{
  //std::cout << "UeMemberLteUeCmacSapProvider::ConfigureRach" << std::endl;
...
{
  //std::cout << "UeMemberLteUeCmacSapProvider::StartContentionBasedRandomAccessProcedure" << std::endl;
...
{
  //std::cout << "UeMemberLteUeCmacSapProvider::StartNonContentionBasedRandomAccessProcedure" << std::endl;
...
{
  //std::cout << "UeMemberLteUeCmacSapProvider::AddLc" << std::endl;
...
{
  //std::cout << "UeMemberLteUeCmacSapProvider::RemoveLc" << std::endl;
...
{
  //std::cout << "UeMemberLteUeCmacSapProvider" << std::endl;
  m_mac->DoReset ();
}
...
{
  //std::cout << "UeMemberLteMacSapProvider::TransmitPdu" << params.D2DFlag << " rnti:" << params.rnti << std::endl;
...
LteUeMac::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
  ...
  */
  ...
  //*********************************************************************************************************************
  bool flag=params.D2DFlag;
  //NS_LOG_UNCOND("LteUeMac::DoTransmitPdu flag:" << flag << " rnti:" << params.rnti);
  LteRadioBearerTag tag;
  if(flag)//the D2D packet
  {
    NS_LOG_UNCOND("LteUeMac::Send a  D2D Packet  -----  src rnti is: " << m_rnti << "  dest rnti is: " << params.targetRnti);
    tag=LteRadioBearerTag(params.targetRnti,params.lcid,0,true);
  }
  else//ordinary LTE packet
  {
    NS_LOG_FUNCTION (this);
    NS_ASSERT_MSG (m_rnti == params.rnti, "RNTI mismatch between RLC and MAC");
    //NS_LOG_UNCOND("the Mac Layer send a LTE packet");
    tag=LteRadioBearerTag(params.rnti, params.lcid, 0 ,false);
  }
  // tag=LteRadioBearerTag(params.rnti, params.lcid, 0 ,false);

  // NS_LOG_UNCOND("LteUeMac::DoTransmitPdu" << params.rnti);
  params.pdu->AddPacketTag (tag);
  //store pdu in HARQ buffer
  m_miUlHarqProcessesPacket.at (m_harqProcessId)->AddPacket (params.pdu);
  m_miUlHarqProcessesPacketTimer.at (m_harqProcessId) = HARQ_PERIOD;
...
LteUeMac::DoAddLc (uint8_t lcId,  LteUeCmacSapProvider::LogicalChannelConfig lcConfig, LteMacSapUser* msu)
{
  ...
  //std::cout << m_rnti << " add" << (int)lcId << std::endl;
}
...
LteUeMac::DoRemoveLc (uint8_t lcId)
{
  ...
  std::cout << "remove" << (int)lcId << std::endl;
}
...
LteUeMac::DoReceivePhyPdu (Ptr<Packet> p)
{
  ...
  //NS_LOG_UNCOND("LteUeMac::DoReceivePhyPdu  flag:" << tag.GetD2DFlag());
      ...
      if (it != m_lcInfoMap.end ())
        {
          ...
          //std::cout << "lcid:" << (int)tag.GetLcid() << " rnti:" << m_rnti << std::endl;
          if(tag.GetD2DFlag())
          {
            NS_LOG_UNCOND("LteUeMac::Receive a D2D Packet  rnti:" << m_rnti );
          }
          it->second.macSapUser->ReceivePdu (p);
        }
      else
        {
          NS_LOG_UNCOND ("received packet with unknown lcid " << (uint32_t) tag.GetLcid ());
        }
    }
}
                              ...
                              || ((*itBsr).second.txQueueSize > 0)))
                        {
                          if ((*itBsr).second.retxQueueSize > 0)
                          ...
                          else if ((*itBsr).second.txQueueSize > 0)
                            {
                              ...
                              //NS_LOG_UNCOND (this << " serve tx DATA, bytes " << bytesForThisLc << ", RLC overhead " << rlcOverhead);

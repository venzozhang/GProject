  ...
  //********************************************************************************************************************************************
{
  ...
  //NS_LOG_UNCOND("LteSpectrumPhy::Constructor()"<< " id:"<< m_test_id);
...
LteSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  // std::cout<<"StartRx : "<<Simulator::Now() << "::  " << m_test_id <<std::endl;
...
//********************************************************************************************************************************************
   /*if( m_useD2DMode)
       NS_LOG_UNCOND("In D2D Mode  "<<m_useD2DMode);
     else
       NS_LOG_UNCOND("In lte mode  "<<m_useD2DMode);*/
  //********************************************************************************************************************************************
...
{
    // zhaojianwu
  // std::cout << " LteSpectrumPhy "<< " Id:" << m_test_id<<" RxData @@@@@@@@@@@@@@@@@" << std::endl;
  NS_LOG_FUNCTION (this);
  switch (m_state)
    {
      case HYBRID:
              ...
              if (params->packetBurst)
                {
                  ...
                  if(m_test_id == 10)
                  {
                    NS_LOG_UNCOND("LteSpectrumPhy::StartRxData() spectrum id: "<< m_test_id << " push D2D packet in m_rxPacketBurstList" );
                  } 
                }
...
{
      // zhaojianwu
  // std::cout << " LteSpectrumPhy  RxCtrl ###########################################" << std::endl;
  NS_LOG_FUNCTION (this);
  switch (m_state)
  {
...
LteSpectrumPhy::EndRxData ()
{
  ...
  // std::cout<<" EndRxData(): state: " << m_state<<"id: "<<m_test_id<<std::endl;
...
// if(m_test_id == 10)
// {
//     // std::cout<<"EndRxData() : "<<"tbsize----------" << m_expectedTbs.size() <<std::endl;
//   expectedTbs_t::iterator it = m_expectedTbs.begin ();
//   while(it != m_expectedTbs.end ())
//   {
//     std::cout << "expectTbs:    rnti ------ " << it->first.m_rnti << "     layer---------- " << (uint16_t)it->first.m_layer<<std::endl;
//     ++it;
//   }
// }//********************************************************************************************************************************************
  ...
  while (itTb!=m_expectedTbs.end ())
      ...
      if ((m_dataErrorModelEnabled)&&(m_rxPacketBurstList.size ()>0)) // avoid to check for errors when there is no actual data transmitted
        {
          ...
          if ((*itTb).second.downlink)
            {
              // DL
              //std::cout << " Dl Rx Data ^&*^&*^&*^&*" << " ID :" << m_test_id << std::endl;
              m_dlPhyReception (params);
            }
          else
            {
              // UL
              //std::cout << " UL Rx Data ^&*^&*^&*^&*" << " ID :" << m_test_id << std::endl;
    ...
    // std::cout<<"EndRxData() : "<<Simulator::Now() << "::  " << m_test_id <<std::endl;
        ...
        for (std::list<Ptr<Packet> >::const_iterator j = (*i)->Begin (); j != (*i)->End (); ++j)
          {
            ...
            // NS_LOG_UNCOND("testid:  " << m_test_id<< "   tbId.m_rnti:  " << tbId.m_rnti);
            ...
            if(m_test_id == 10)
            {
              ...
              {
                //NS_LOG_UNCOND("LteSpectrumPhy::EndRxData()  spectrum id:  " << m_test_id << " Donn't find the expectTbs, that's too bad");
              }
            }
                ...
                if (!(*itTb).second.corrupt)
                  {
                      ...
                      if (!m_ltePhyRxDataEndOkCallback.IsNull ())
                      {
                        ...
                        if(m_test_id == 10)
                        {
                          NS_LOG_UNCOND("LteSpectrumPhy::EndRxData()  spectrum id:  " << m_test_id << " The ExpectTbs is found, spectrumPhy has received  and deliverd D2D packet to MAC");
                        }
                        m_ltePhyRxDataEndOkCallback (*j);
...
LteSpectrumPhy::EndRxDlCtrl ()
{
  ...
  //std::cout<<" EndRxDlCtrl(): state: " << m_state<<"id: "<<m_test_id<<std::endl;
      ...
      if (!m_ltePhyRxCtrlEndOkCallback.IsNull ())
        {
          ...
          // std::cout << " LteSpectrumPhy::EndRxDlCtrl " << std::endl;
          m_ltePhyRxCtrlEndOkCallback (m_rxControlMessageList);
        }
    }
  else
...
{
  // std::cout<<" EndRxUlSrs(): state: " << m_state<<"id: "<<m_test_id<<std::endl;
  NS_ASSERT (m_state == HYBRID);
  ChangeState (IDLE);
  m_interferenceCtrl->EndRx ();
...
//********************************************************************************************************************************************
void
LteSpectrumPhy::UseD2DMode(bool flag)
{
  //NS_LOG_UNCOND("LteSpectrumPhy::UseD2DMode (): " << flag);
  m_useD2DMode = flag;
} 

...
//********************************************************************************************************************************************
  void DoUseD2DMode (bool flag, uint64_t targetImsi, uint16_t targetRnti);



  Ptr<LteSpectrumPhy>  GetD2DSendSpectrumPhy();

  Ptr<LteSpectrumPhy>  GetD2DRecvSpectrumPhy();


  void SetD2DMacPdu (Ptr<Packet> p);
  Ptr<PacketBurst> GetD2DPacketBurst (void);

  Ptr<LteSpectrumPhy> m_d2dsendlinkSpectrumPhy;
  Ptr<LteSpectrumPhy> m_d2drecvlinkSpectrumPhy;
  Ptr<SpectrumChannel> m_d2dChannel1;
  Ptr<SpectrumChannel> m_d2dChannel2;
  bool  m_useD2DMode;
  uint64_t  m_targetImsi;
  uint16_t m_targetRnti;


  std::vector< Ptr<PacketBurst> > m_d2dPacketBurstQueue;

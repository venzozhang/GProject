...
LteRlcUm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
  ...
  //std::cout << "LteRlcUm::DoTransmitPdcpPdu flag: " << D2DFlag << std::endl;
...
LteRlcUm::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId)
{
  ...
  params.D2DFlag = D2DFlag;
  //std::cout << "LteRlcUm::DoNotifyTxOpportunity " << D2DFlag << std::endl; 
  if(D2DFlag)
  {
    NS_LOG_UNCOND("LteRlcUm::Send a  D2D Packet  -----  src rnti is: " << m_rnti << "  dest rnti is: " << m_targetRnti);
  }
  /*******************************************************************/
...
{
  NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize ());
  //std::cout << "LteRlcUm::DoReceivePdu " << std::endl;
  // Receiver timestamp
  RlcTag rlcTag;
  Time delay;

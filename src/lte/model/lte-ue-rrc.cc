...
LteUeRrc::DoInitialize (void)
{
  ...
  // lcConfig.priority = 10;
  // lcConfig.prioritizedBitRateKbps = 10000;
  // lcConfig.bucketSizeDurationMs = 1000;
  // lcConfig.logicalChannelGroup = 2;      
  // std::cout << m_rnti << " add 100" << std::endl;
  // m_cmacSapProvider->AddLc (100,lcConfig,rlc->GetLteMacSapUser ());
}
...
// NS_LOG_UNCOND("LteUeRrc::DoSendData()  send D2D data");
  std::cout<<"LteUeRrc::DoSendData()  send D2D data"<<std::endl;
...
{
  NS_LOG_FUNCTION (this);
  std::cout << "LteUeRrc::DoReceivePdcpSdu" << std::endl;
...
LteUeRrc::DoNotifyRandomAccessSuccessful ()
{
  ...
  // NS_LOG_UNCOND("hhhhhhhhhhhhhhhhhh");
  NS_LOG_UNCOND("LteUeRrc::DoNotifyRandomAccessSuccessful:  ");
...
void 
LteUeRrc::DoRecvD2DConnectionSetup (LteRrcSap::D2DConnectionSetup msg)
{
  NS_LOG_UNCOND("LteUeRrc::DoRecvD2DConnectionSetup()  -----  rnti is: " << m_rnti);
  //std::cout << Simulator::GetContext() << " " <<  std::endl;
  if(msg.rnti != m_rnti)
  {
    m_cphySapProvider -> UseD2DMode(true, 2, msg.rnti);
    m_IpToRnti["firstIp"] = msg.rnti;
    m_useD2DMode = true;

    if(m_useD2DMode) 
    {
      //NS_LOG_UNCOND("setup d2d connection");
      Ptr<Packet> packet = Create<Packet> ();

      std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator itd2d =   m_drbMap.find (100);
      if(itd2d != m_drbMap.end()) 
      {
        LtePdcpSapProvider::TransmitPdcpSduParameters paramsd2d;
        paramsd2d.pdcpSdu = packet;
        //std::cout << "bbbb" << std::endl;
        paramsd2d.rnti = m_rnti;
        paramsd2d.lcid = itd2d->second->m_logicalChannelIdentity;
        paramsd2d.D2DFlag = true;
        //std::cout << "rnti:" << m_rnti << " target rnti:" << m_IpToRnti["firstIp"] << std::endl;
        paramsd2d.targetRnti = m_IpToRnti["firstIp"]; 
        NS_LOG_UNCOND("LteUeRrc::Send a  D2D Packet  -----  src rnti is: " << m_rnti << "  dest rnti is: " << paramsd2d.targetRnti);
        //Time now = now();
        //ScheduleWithContext(2,Seconds(1.2),LtePdcpSapProvider::TransmitPdcpSdu,itd2d->second->m_pdcp->GetLtePdcpSapProvider (),paramsd2d);
        itd2d->second->m_pdcp->GetLtePdcpSapProvider ()->TransmitPdcpSdu (paramsd2d);
      }
    }
  }
  else if(msg.rnti == m_rnti)
  {
    ...
    drbInfo->m_rlc = rlc;
    drbInfo->m_epsBearerIdentity = 0;
    drbInfo->m_logicalChannelIdentity = 100;
    drbInfo->m_drbIdentity = 100;
    ...
    // m_bid2DrbidMap[dtamIt->epsBearerIdentity] = 100;

    m_drbMap.insert (std::pair<uint8_t, Ptr<LteDataRadioBearerInfo> > (100, drbInfo));
      ...
      if (m_srb1 == 0)
        {
          ...
          //std::cout << (int)lcConfig.priority << " " << (int)lcConfig.prioritizedBitRateKbps << " " << (int)lcConfig.bucketSizeDurationMs << " " << (int)lcConfig.logicalChannelGroup << std::endl;
          m_cmacSapProvider->AddLc (lcid, lcConfig, rlc->GetLteMacSapUser ());
...
LteUeRrc::ConnectionTimeout ()
{
  NS_LOG_UNCOND("LteUeRrc::ConnectionTimeout");
  NS_LOG_FUNCTION (this << m_imsi);
  m_cmacSapProvider->Reset ();       // reset the MAC
  m_hasReceivedSib2 = false;         // invalidate the previously received SIB2
  SwitchToState (IDLE_CAMPED_NORMALLY);
...
//********************************************************************************************************************************************
//D2D  specified function

void 
LteUeRrc::SetupD2DConnection(uint64_t dstUeIdentity)
{
        LteRrcSap::D2DConnectionRequest msg;
        // msg.srcUeIdentity = m_imsi;
        msg.dstUeIdentity = dstUeIdentity;
        ...
        // m_rrcSapUser->SendD2DConnectionRequest (msg); 
        //std::cout << "LteUeRrc::SetupD2DConnection" << std::endl;
        m_connectionTimeout = Simulator::Schedule (Seconds(0.5),
                                                   &LteUeRrcSapUser::SendD2DConnectionRequest,
                                                   m_rrcSapUser,msg);
}

...
LteRadioBearerTag::LteRadioBearerTag (uint16_t rnti, uint8_t lcid, uint8_t layer,bool flag)
: m_rnti (rnti),
  m_lcid (lcid),
  m_layer (layer)
{
...
void LteRadioBearerTag::SetD2DFlag(bool flag)
{
  if(flag)
    m_D2DFlag=1;
  else
...
bool
LteRadioBearerTag::GetD2DFlag() const
{
  if(m_D2DFlag==1)
    return true;
  else

...
/************************************************************/
void
LteRlc::SetTargetRnti (uint16_t targetRnti)
{
  NS_LOG_FUNCTION (this << (uint32_t) targetRnti);
  m_targetRnti = targetRnti;
}

void
LteRlc::SetD2DMode(bool flag)
{
  NS_LOG_FUNCTION (this << (uint32_t) flag);
  D2DFlag = flag;
}
/*************************************************************/
void
...
LteRlcSm::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId)
{
  ...
  std::cout << "LteRlcSm:: send pdu to mac flag:" << D2DFlag << " rnti:" << m_rnti <<std::endl;

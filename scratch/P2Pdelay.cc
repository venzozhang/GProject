...
std::map<Ptr<Node>, std::pair<uint32_t, double> > p2pDelay;
...
Stats (void)
{
  ...
  uint32_t receivesNumber = 0;
  double totalDelay = 0;
  for (NodeContainer::Iterator i = nodes.Begin (); i != nodes.End (); ++i)
  {
    Ptr<Node> receiveNode = (*i);
    int id = receiveNode->GetId();
    receivesNumber += p2pDelay[receiveNode].first;
    totalDelay += p2pDelay[receiveNode].second;
    std::cout << id << " " << p2pDelay[receiveNode].second/p2pDelay[receiveNode].first << std::endl;
  }
  double meanDelay = totalDelay / receivesNumber;
